...
  // State for Software Update fields
  const [softwareUpdate, setSoftwareUpdate] = useState({
    feature: "softwareupate",
...
  // State for Security Update fields (disabled)
...
      return;
    }
//     console.log(activeMac) // Log activeMac for debugging, as it was in your original code
...
    setPanValue(value);
    // sendCommand('pan', { value }); // This is commented out in your original code
  };

  const handleTiltChange = (e) => {
    const value = Number(e.target.value);
    setTiltValue(value);
    // sendCommand('tilt', { value }); // This is commented out in your original code
  };
...
                return;
              }
              // Ensure the feature name matches what your backend expects for software updates
              const payload = { ...softwareUpdate };
              sendCommand("softwareupate", payload); // 'softwareupate' as per your original example JSON
              showToast('success', 'OTA update command sent.');
            }}
            className={`${clientId ? 'bg-blue-600 hover:bg-blue-700' : 'bg-blue-300 cursor-not-allowed'} text-white font-medium py-2 px-4 rounded transition w-fit`}
            disabled={!clientId}
          >
            OTA
          </button>
...
        {/* Security Update Section (UI Only) */}
        <div className="flex flex-col gap-3 p-4 border rounded-lg shadow-sm col-span-full cursor-not-allowed">
          <h3 className="text-lg font-medium ">Security Update</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {["server", "user", "pass", "filepath", "type"].map((field) => (
              <div key={field} className="flex flex-col gap-1">
                <label className="text-sm  capitalize" htmlFor={`security-${field}`}>
                  {field === "pass" ? "Password" : field.charAt(0).toUpperCase() + field.slice(1)}
                </label>
                <input
                  id={`security-${field}`}
...
          </div>
          <button
            className="mt-3 bg-gray-500 opacity-50 text-white font-medium py-2 px-4 rounded transition w-fit cursor-not-allowed"
            disabled
          >
            Submit
          </button>
